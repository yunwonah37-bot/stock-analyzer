...
@app.route("/api/npv/<code>")
def get_npv_data(code):
    """NPV 계산용 과거 10년치 영업이익 + 발행주식수 + 순부채 반환."""
    if not DART_KEY:
        return jsonify({"error": "DART API 키 없음"}), 400
    ...
    def _fetch_year(year):
        for fs_div in ("CFS", "OFS"):
            ...
            if data and data.get("list"):
                ...
                op_it = _find_acct(idx, IS_SECTS, ["영업이익", "영업이익(손실)"])
                if op_it:
                    val = _to_ok(op_it.get("thstrm_amount", 0))
                    if val != 0:
                        return year, val
        return year, None

    target_years = list(range(cur_year - 10, cur_year))

    from concurrent.futures import ThreadPoolExecutor, as_completed
    with ThreadPoolExecutor(max_workers=5) as ex:
        futs = {ex.submit(_fetch_year, y): y for y in target_years}
    ...
    op_list      = [results[y] for y in years_sorted]

    shares   = _fetch_dart_shares(code)
    net_debt = None
    dart_fin = get_dart_financials(code)
    if dart_fin:
