...
function loadCompany(code) {
  ...
  document.getElementById('npvContent').innerHTML    = '';
...
function renderOverview(data) {
  ...
  window.stockData = { price: info.current_price || 0 };
  const r    = data.ratios;
...
function switchTab(tab, animate = true) {
  ...
  if (tab === 'npv'    && !npvLoaded    && currentCode) loadNPVData();
}
...
/* ── NPV 밸류에이션 탭 ──────────────────────────────────────── */
async function loadNPVData() {
  const container = document.getElementById('npvContent');
  container.innerHTML = `
    <div class="card" style="text-align:center;padding:48px;color:var(--text-muted)">
      <div style="font-size:24px;margin-bottom:10px">⏳</div>
      <div>DART에서 10년치 영업이익 불러오는 중...<br><small style="font-size:12px;margin-top:6px;display:block">연도별 사업보고서를 순차 조회합니다 (10~20초 소요)</small></div>
    </div>`;
  try {
    const data = await fetch(`/api/npv/${currentCode}`).then(r => r.json());
    ...
    container.innerHTML = `
      <div class="card">
        <div class="card-header">NPV 밸류에이션</div>
        <div class="rnd-nodata">
          <div class="rnd-nodata-icon">⚠️</div>
          <div class="rnd-nodata-title">데이터 로드 실패</div>
          <div class="rnd-nodata-desc">${e.message}</div>
        </div>
      </div>`;
  }
}

function renderNPV(data) {
  const container = document.getElementById('npvContent');
  const years     = data.years || [];
  const opList    = data.operating_profit || [];
  let sharesRaw   = data.shares || 0;
      ...
      <div class="card">
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:20px">
          ...
          <div><label style="font-size:12px;color:var(--text-muted);display:block;margin-bottom:6px">발행주식수 (만주)</label>
            <input type="number" id="npvShares" value="${Math.round(sharesRaw/10000)}" step="1" style="width:100%;padding:8px;border:1px solid var(--border);border-radius:6px;background:var(--bg);color:var(--text);font-size:14px"></div>
        ...
        <div style="overflow-x:auto;margin-bottom:20px">
          <table style="width:100%;border-collapse:collapse;font-size:13px">
            <thead><tr style="border-bottom:1px solid var(--border)">
              <th style="text-align:left;padding:8px;color:var(--text-muted);font-weight:500">연도</th>
              <th style="text-align:right;padding:8px;color:var(--text-muted);font-weight:500">추정 영업이익</th>
              <th style="text-align:right;padding:8px;color:var(--text-muted);font-weight:500">현재가치(PV)</th>
              <th style="text-align:right;padding:8px;color:var(--text-muted);font-weight:500">누적 PV</th>
            </tr></thead>
            <tbody>${tRows}</tbody>
          </table>
        </div>
...
window.npvRecalc = function() { npvLoaded = false; loadNPVData(); };
